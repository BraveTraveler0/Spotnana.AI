...
**Phone (Android APK, built 2026-09-21): needs two routes from Iris.** The phone has no `iris-feed\` folder, so it keeps a copy in its own storage, pulled over the **Hermes gateway on Tailscale**. **Iris: please serve on the gateway** (same bearer key as `/v1/runs`) `GET /artemis/feed` (a JSON bundle of `latest.json`, the files it names, `weekly-review.md`, the spark and `weekly-goals.json`) and `POST /artemis/checkoffs` (append to `iris-feed\checkoffs.jsonl`). The exact spec, three questions for Iris and the state of her briefs are in `CLAUDE-REPLY-android-and-event-cards.md` (repo root). The phone talks only to horus's Ollama (`100.94.158.103:11434`) and the gateway, both over Tailscale; no cloud model is ever used on it (§0 unchanged).
