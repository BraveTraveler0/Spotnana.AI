...
function explicitDate(text: string, today: Date): Date | null {
  for (const match of text.replace(DATE_RANGE, ' ').matchAll(MONTH_DAY)) {
...
export function inferWhen(text: string, now: Date = new Date()): string | null {
  // "already passed" means the date in the text is history, not a plan.
  if (/already passed/i.test(text)) return null;
  ...
  if (!date) {
    ...
    let ahead = (weekday - today.getDay() + 7) % 7;
    // Today's weekday, but that time has gone by: the next one.
    if (ahead === 0 && minutes !== null && now.getHours() * 60 + now.getMinutes() >= minutes) ahead = 7;
...
const DAY_NAME = '(?:mon(?:day)?|tue(?:s|sday)?|wed(?:nesday)?|thu(?:r|rs|rsday)?|fri(?:day)?|sat(?:urday)?|sun(?:day)?)s?';
...
export function isEvergreen(text: string): boolean {
  return /\b(?:weekly|daily|minimum|routine)\b|\/week\b|\bper week\b|\ba week\b|\bevery day\b/i.test(text);
}
